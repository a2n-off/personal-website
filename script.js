document.addEventListener('DOMContentLoaded', function() {
    ...
    function animation() {
        ...
        ftl.to(footer, {y: 0, stagger: 0.05, autoAlpha: 1, duration: speedDuration, ease: 'expoScale(0.5,7,none)'}, 'tag');
        footer.childNodes.forEach((childNode) => {
            childNode.childNodes.forEach((p) => {
                p.innerHTML = p.textContent.replace(/\S/g, "<span class='letter'>$&</span>");
                p.childNodes.forEach((span, i) => {
                    if (i === 0) {
                        ftl.to(span, {y: 0, stagger: 0.05, autoAlpha: 1, duration: speedDuration, ease: 'expoScale(0.5,7,none)'}, 'tag');
                    } else {
                        ftl.to(span, {y: 0, stagger: 0.05, autoAlpha: 1, duration: speedDuration, ease: 'expoScale(0.5,7,none)'}, `<${0.003}`);
                    }
                })
            })
        })
        ...
        async function processImages() {
            for (const img of images) {
                ...
                textureLoader.load(imgSrc, function (imgTexture) {
                    ...
                    tl.to(canvas, {x: 0, autoAlpha: 1, stagger: 0.05, duration: 0.5, ease: 'expoScale(0.5,7,none)'});
                });
            }
        }
        processImages();
